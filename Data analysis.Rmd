...
for (natives_files in list.files(path = "drive-download-20230118T144007Z-001/fastas/all/natives")){
  native_file <- read.FASTA(paste0("drive-download-20230118T144007Z-001/fastas/all/natives/", natives_files))
...
sub_raw_mitogenomes <- subset(raw_mitogenomes, `Population/Categories` %in% c("admix.arg", "admix.Bra", "ancient.arg", "ancient.Bra", "Je", "Arawak", "Tupi", "Native2"))
...
Haplogroup_freq_subpopulations <- ggplot(sub_raw_mitogenomes, aes(`Population/Categories` , fill = substr(`haplogroups(n)`, 1, 2))) +
  ...
  scale_x_discrete(limits = c("admix.arg", "admix.Bra", "ancient.arg", "ancient.Bra", "Je", "Arawak", "Tupi", "Native2"), labels = c("Admixed Argentinian", "Admixed Brazilian", "Ancient Argentinian", "Ancient Brazilian", "Je", "Arawak", "Tupi", "Native2")) +
  coord_flip() +
  theme_minimal() +
...
norm_Haplogroup_freq_subpopulations <- ggplot(sub_raw_mitogenomes, aes(`Population/Categories` , fill = substr(`haplogroups(n)`, 1, 2))) +
  ...
  scale_fill_discrete(name = "Haplogroups") +
  scale_x_discrete(limits = c("admix.arg", "admix.Bra", "ancient.arg", "ancient.Bra", "Je", "Arawak", "Tupi", "Native2"), labels = c("Admixed Argentinian n = 93", "Admixed Brazilian n = 711", "Ancient Argentinian n = 38", "Ancient Brazilian n = 40", "Je n = 32", "Arawak n = 27", "Tupi n = 44", "Rest of natives \n grouped n = 67")) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Haplogroup composition", y = "% individuals for each haplogroup", x = "Population")
...
norm_Haplogroup_freq_populations <- ggplot(raw_mitogenomes, aes(`Population/Categories2` , fill = substr(`haplogroups(n)`, 1, 2))) +
  geom_bar(position = "fill") +
  scale_fill_discrete(name = "Haplogroups") +
  scale_x_discrete(limits = c("Native", "aDNA", "admixed"), labels = c("Native n = 170", "aDNA n = 78", "admixed n = 804")) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Haplogroup composition", y = "% individuals for each haplogroup", x = "Population")
...
beeData <- read.FASTA("drive-download-20230118T144007Z-001/todos.fa")
class(beeData)

#file natives does not contain sequences from tupi, arawak and Je
#Native file inside fastasforhaplotypes NOT THE SAME!!!
...
dim(id_population)
table(id_population$Population)

#write.table(id_population, "id_population", sep = "\t")
...
class(diferentiation_matrix)

rownames(diferentiation_matrix) <- c("Admixed Argentinian", "Admixed Brazilian", "Ancient Argentinian", "Ancient Brazilian", "Arawak", "Je", "Rest of natives \n grouped", "Tupi")
...
vector_populations <- c("adm.arg", "adm.Bra", "anct.arg", "anct.Bra", "Je", "Arawak", "Native2", "Tupi")


png(filename = "Heatmap_distances.png", width = 1000, height = 800, res = 150)
heatmapSpp(diferentiation_matrix, vector_populations, triangle = "lower", showData = TRUE, dataRound = 1, dataCEX = 0.8)
dev.off()
```
...
vector_populations <- c("admix.arg", "admix.Bra", "ancient.arg", "ancient.Bra", "Je", "Arawak", "Tupi", "Native2")
...
l_shared_haplogroups <- gather(m_shared_haplogroups, pop_2, counts, admix.arg:Native2)
...
png(filename = "Je_donor.png", width = 1000, height = 800, res = 150)
ggplot(subset(l_shared_haplogroups, vector_populations == "Je") , aes(x = pop_2, y = normalized_counts, fill = pop_2)) +
  geom_bar(stat = "identity", position = "stack" ) +
  scale_x_discrete(limits = c("admix.arg", "admix.Bra", "ancient.arg", "ancient.Bra", "Je", "Arawak", "Tupi", "Native2"), labels = c("Admixed Argentinian", "Admixed Brazilian", "Ancient Argentinian", "Ancient Brazilian", "Je", "Arawak", "Tupi", "Rest of natives \n grouped")) +
  theme_minimal() +
  ...
  theme(axis.text.x = element_text(angle = 35, vjust = 0.5), axis.title.y = element_text(angle = 0, vjust = 0.5), legend.position = "none") +
  scale_fill_viridis_d()
dev.off()


png(filename = "Arawak_donor.png", width = 1000, height = 800, res = 150)
...
png(filename = "ancient.Bra_donor.png", width = 1000, height = 800, res = 150)
...
png(filename = "ancient.arg_donor.png", width = 1000, height = 800, res = 150)
ggplot(subset(l_shared_haplogroups, vector_populations == "ancient.arg") , aes(x = pop_2, y = normalized_counts, fill = pop_2)) +
  geom_bar(stat = "identity", position = "stack" ) +
  scale_x_discrete(limits = c("admix.arg", "admix.Bra", "ancient.arg", "ancient.Bra", "Je", "Arawak", "Tupi", "Native2"), labels = c("Admixed Argentinian", "Admixed Brazilian", "Ancient Argentinian", "Ancient Brazilian", "Je", "Arawak", "Tupi", "Rest of natives \n grouped")) +
  theme_minimal() +
  labs(title = "Haplogroup sharing", subtitle = "Donor population (Ancient Argentina)", y = "% of sequences from \n Acceptor populations \n that share haplogroups \n with the Donor populations", x = "Acceptor populations") +
  theme(axis.text.x = element_text(angle = 35, vjust = 0.5), axis.title.y = element_text(angle = 0, vjust = 0.5), legend.position = "none") +
  scale_fill_viridis_d()
